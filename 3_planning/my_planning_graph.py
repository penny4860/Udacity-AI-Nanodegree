...
class PlanningGraph():
    ...
    def _is_duplicated(self, effects_1: list, effects_2: list) -> bool:
        """Checks for the existence of duplicated elements """
        for e1 in set(effects_1):
            if e1 in set(effects_2):
                return True
        return False
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if self._is_duplicated(action1.effect_add, action2.effect_rem) or \
            self._is_duplicated(action2.effect_add, action1.effect_rem):
            return True
        else:
            return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if self._is_duplicated(action1.effect_add, action2.precond_neg) or \
            self._is_duplicated(action1.effect_rem, action2.precond_pos) or \
            self._is_duplicated(action2.effect_add, action1.precond_neg) or \
            self._is_duplicated(action2.effect_rem, action1.precond_pos):
            return True
        else:
            return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Check mutual exclusion between nodes
...
#                     return i
    ...
    def h_levelsum(self) -> int:
        ...
        # for each goal expression
        for goal in self.problem.goal:
            goal_node = PgNode_s(goal, True)
            level_for_a_goal = 0
            # count number of levels until reaching one goal expression
            for s_level in self.s_levels:
                if goal_node in s_level:
                    break
                level_for_a_goal += 1
            level_sum += level_for_a_goal
